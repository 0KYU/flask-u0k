...
| **修复验证** |
| V4 | 输入校验 | `&&` 元字符拒绝 | `127.0.0.1 && whoami` | 返回 "目标格式不合法" | ✅ 被拒绝 |
| V5 | 输入校验 | `;` 元字符拒绝 | `127.0.0.1; dir` | 返回 "目标格式不合法" | ✅ 被拒绝 |
| V6 | 输入校验 | `\|` 管道拒绝 | `8.8.8.8 \| dir` | 返回 "目标格式不合法" | ✅ 被拒绝 |
